...
3. For user installed applications that live in `$HOME/.local/bin` to be available, `$HOME/.local/bin` should be in the `$PATH` environment variable. Ubuntu's default `.profile` already ships with blocks that add `$HOME/bin` and `$HOME/.local/bin` to `PATH`, but they sit *after* the `# if running bash` block that sources `.bashrc` - so when `.bashrc` runs `oh-my-posh init`, `.local/bin` isn't on the `PATH` yet. Check the order in your `.profile`:

    ```bash
    cat "$HOME/.profile"
    ```

    If the two `PATH` blocks already appear *before* `# if running bash`, skip to step 4. Otherwise, follow [Recipe 2: Reordering existing blocks](../additional-info/profile-vs-bashrc-wsl2.md#recipe-2-reordering-existing-blocks) in the `.profile` vs `.bashrc` guide to move them above it (or add the `$HOME/.local/bin` block if it's missing entirely).
